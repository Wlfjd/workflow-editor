...
**직접 결정**: 외부 그래프 라이브러리 미사용, 히스토리 50단계 제한, 노드 타입 3종(입력/처리/출력) 채택.
...
**직접 수행**: 최종 diff 리뷰, 불필요한 추상화 제거, 로고 원본 에셋 선택.
...
| 주제 | AI 활용 내용 | 결과물 |
| --- | --- | --- |
| 전체 UI 톤 | 다크 테마 팔레트, 패널/캔버스/그리드 배경 | `index.css` (`:root` 변수) |
| 노드 카드 | 타입별 accent 색, 선택 링, 헤더·본문 레이아웃 | `NodeView.tsx`, `constants.ts`, `index.css` |
| 포트·연결 UX | 포트 펄스·호버(초록/빨강) 애니메이션, 연결 미리보기 점선 | `index.css`, `Canvas.tsx` |
| 엣지 | 베지어 곡선, 선택/호버 색, SVG 화살표 marker | `EdgeView.tsx`, `index.css` |
| 툴바·상태바 | 버튼 그룹, 줌 표시, 브랜드 영역 레이아웃 | `Toolbar.tsx`, `StatusBar.tsx`, `index.css` |
| 노드 추가 다이얼로그 | 모달 백드rop, 입력 필드, primary 버튼 스타일 | `NodeAddDialog.tsx`, `index.css` |
| 브랜딩 | 로고 `<img>` 적용, favicon 연동, 크기·세로 배치, `JIWON's Workflow` 텍스트 | `Toolbar.tsx`, `index.css`, `index.html` |
| 토스트 | 하단 알림 애니메이션 | `index.css`, `App.tsx` |

**직접 수행**: 로고·브랜드 이미지(`public/sss.png` 등) **제작·선택**, 최종 크기·문구 확정, Vercel 배포 후 실기기에서 가독성 확인.

**사용하지 않은 것**: Figma-to-code 플러그인, UI 전용 AI( v0 등) — CSS와 컴포넌트 마크업을 Agent 세션에서 직접 수정하는 방식으로 통일.
...
| 6 | AI 활용 내역 구체적 문서화 | `docs/AI_USAGE.md` 작성 | `AI_USAGE.md` |
| 7 | 로고 크기·브랜드 텍스트 조정 | height 38px, `JIWON's Workflow` 하단 텍스트 | `index.css`, `Toolbar.tsx` |
| 8 | 노드 추가 다이얼로그·빈 캔버스 | 제목/설명 입력 UI, 초기 그래프 비움 | `NodeAddDialog.tsx`, `useWorkflow.ts`, `App.tsx` |
...
## 7. AI 기여 vs 직접 기여 (대략적 구분)

| 구분 | AI 기여 | 직접 기여 |
| --- | --- | --- |
| **아키텍처** | 대안 제시, 문서화, 보일러플레이트 | 라이브러리 미사용 결정, 범위 설정 |
| **디자인** | 다크 테마·컴포넌트 CSS, 연결 피드백, 툴바/다이얼로그 레이아웃, 로고 배치 | 로고 **원본 이미지** 선택·제작 |
